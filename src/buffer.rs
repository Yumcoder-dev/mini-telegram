...
///
/// The `Client` performs MTProto commands directly on the TCP connection. Only a
/// single request may be in-flight at a given time and operations require
/// mutable access to the `Client` handle. This prevents using a single MTProto
/// connection from multiple Tokio tasks.
///
/// The strategy for dealing with this class of problem is to spawn a dedicated
/// Tokio task to manage the MTProto connection and using "message passing" to
...
/// MTProto connection. When the response is received, it is forwarded to the
/// original requester.
///
