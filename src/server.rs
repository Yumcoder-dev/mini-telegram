...
mod tests {
    ...
    #[tokio::test]
    async fn test_note_about_move_in_run() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();

        let (notify_shutdown, _) = broadcast::channel(1);
        let (shutdown_complete_tx, shutdown_complete_rx) = mpsc::channel(1);

        // TODO: why needs to pass shutdown_complete_rx to listener
        let server = Listener {
            listener,
            db_holder: DbDropGuard::new(),
            limit_connections: Arc::new(Semaphore::new(MAX_CONNECTIONS)),
        ...
        // uncomment the following lines to get error
        // drop(notify_shutdown); // value used here after move
        // drop(shutdown_complete_tx); // value used here after move
        // let _ = shutdown_complete_rx.recv().await; // value used here after move
    ...
    #[tokio::test]
    async fn test_why_we_need_db_drop_guard() {
        // 1) when Listener drop (after run function terminated)
        // 2) then the db field (DbDropGuard) also drop
        // 3) back ground purge task is stopped
    }
}
